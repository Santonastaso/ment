  ...
  return (
    <div className="border border-gray-300 rounded-lg px-3 py-2 focus-within:ring-2 focus-within:ring-navy-light focus-within:border-transparent bg-white min-h-[44px]">
      <div className="flex flex-wrap gap-2 items-center">
        {value.map((skill, i) => (
          <span key={i} className="flex items-center gap-1 bg-blue-50 text-navy-light border border-blue-200 rounded-full px-3 py-0.5 text-sm font-medium">
            {skill}
            <button
              type="button"
              onClick={() => removeSkill(i)}
              className="text-blue-400 hover:text-navy ml-0.5 leading-none"
            >
              ×
            </button>
          </span>
