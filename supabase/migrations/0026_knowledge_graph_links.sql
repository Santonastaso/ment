-- 0026 Knowledge graph: real-relationship edges + school fields (Wave 4)
--
-- Extends knowledge_graph() (0016, signature unchanged):
--   * Person nodes now carry `program`, `cohort_year` and `role` (persona)
--     so admins can filter/group by program client-side.
--   * New person->person edges — the "connections that actually happened":
--       type 'session'    = completed mentoring sessions (weight = count)
--       type 'connection' = accepted connections        (weight = count)
--     Pairs are canonicalized (least/greatest id) and aggregated per pair.
--     Both endpoints must be inside the current scope (org/language filters).
--     Self-pairs are excluded defensively.
...
begin
  ...
  with people as (
    select p.id, p.name, p.department, p.seniority, p.job_title, p.location,
           ...
           p.program, p.cohort_year, p.role,
  ...
  real_edges as (
    ...
    from (
      select least(s.mentor_id, s.mentee_id) as a,
             greatest(s.mentor_id, s.mentee_id) as b,
             'session' as link_type,
             count(*) as w
      from public.sessions s
      join people pm on pm.id = s.mentor_id
      join people pn on pn.id = s.mentee_id
      where s.status = 'completed'
        and s.mentor_id <> s.mentee_id
      group by 1, 2, 3
      union all
      select least(c.requester_id, c.addressee_id),
             greatest(c.requester_id, c.addressee_id),
             'connection',
             count(*)
      from public.connections c
      join people pr on pr.id = c.requester_id
      join people pa on pa.id = c.addressee_id
  ...
  select
    ...
    (select coalesce(jsonb_agg(edge), '[]'::jsonb)
       from (select edge from edges
             union all
             select edge from real_edges) e)
